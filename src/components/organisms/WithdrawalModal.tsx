...
import { Animated, Modal, NativeSyntheticEvent, StyleSheet, Text, TextInput, TextInputChangeEventData, View } from 'react-native';
            ...
            <View style={styles.back} accessible={false}>
                <Animated.View style={[styles.containerView, { bottom: upValue }]}>
                    <TouchableOpacity style={styles.containerTouchableOpacity} activeOpacity={1}>
                        ...
                        <Text style={styles.title2}>한번 탈퇴하면 3개월간 재가입할 수 없어요{'\n'}탈퇴를 원하시면 아래에 탈퇴라고 적어주세요</Text>
                        <TextInput style={[styles.textInput, { borderColor: textInputBorderColor }]} placeholder='탈퇴' value={text} onChangeText={setText} onChange={onChange} />
                        <TouchableOpacity onPress={onPress} disabled={buttonDisabled}>
                            <LinearGradient style={styles.linearGradient} colors={buttonDisabled ? ['#F5F5FB', '#F5F5FB'] : ["#947BEA", "#1151E5"]}>
                                <Text style={[styles.buttonText, { color: buttonDisabled ? '#6F81A9' : '#fff' }]}>탈퇴하기</Text>
                            </LinearGradient>
                        </TouchableOpacity>
                    </TouchableOpacity>
                </Animated.View>
