...
export default function Complaint({ navigation }: StackScreenProps<SettingNavParamList>) {
    ...
    return <SafeAreaView style={styles.safeAreaView}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: 20 }}>
            ...
            <TextInput style={[styles.textInput, { height: 280 }]} value={content} onChangeText={setContent} multiline={true} />
        </ScrollView>
        <TouchableOpacity style={[styles.button, { backgroundColor: buttonDisabled ? '#F5F5FB' : '#044DE4' }]}
            disabled={buttonDisabled} onPress={() => navigation.goBack()}>
            <Text style={[styles.buttonText, { color: buttonDisabled ? '#6F81A9' : '#fff' }]}>전송하기</Text>
        </TouchableOpacity>
    </SafeAreaView>
}
